...
namespace Api.Manage.Assist.Utils
{
  public class ValidateCode
  {
    ...
    public void CreateValidateGraphic(string validateCode, HttpContext context)
    {
      ...
      try
      {
        ...
        context.Response.WriteAsync(stream.ToString());
    ...
    {
      Bitmap image = new Bitmap((int) Math.Ceiling(validateCode.Length * 23.0), 28);
      Graphics g = Graphics.FromImage(image);
      try
      {
        //生成随机生成器
        ...
        LinearGradientBrush brush = new LinearGradientBrush(
          new System.DrawingCore.Rectangle(8, image.Height / 2, image.Width, image.Height),
          System.DrawingCore.Color.Blue, System.DrawingCore.Color.DarkRed, 1.2f, true);
        g.DrawString(validateCode, font, brush, 3, 2);
        //画图片的前景干扰点
        for (int i = 0; i < 100; i++)
